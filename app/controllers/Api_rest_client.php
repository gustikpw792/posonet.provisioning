...
class Api_rest_client extends CI_Controller
{
	...
	public function los()
	{
		...
		foreach ($req as $br) {
			...
			$viewOnMap = '<a href="javascript:void(0)" class="btn btn-xs btn-outline btn-info" title="Show On Map" onclick="show_on_map(' . $br->no_pelanggan . ')"><i class="fa fa-map"></i> ' . $br->odp_name . ' </a>';
			$row[] = $viewOnMap;
